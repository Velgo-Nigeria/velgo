...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    ...
    switch (view) {
      ...
      case 'overview': return <Overview profile={profile} onRefreshProfile={() => fetchProfile(session.user.id)} onUpgrade={() => navigate('subscription')} onViewLegal={(tab) => navigate('legal', tab)} onShowGuide={() => setShowGuide(true)} onShowNotifications={() => setShowNotifications(true)} unreadCount={unreadNotificationsCount} onNavigate={navigate} />;
      ...
      case 'task-detail': return <TaskDetail profile={profile} taskId={viewData} onBack={() => handleBackNavigation('home')} onUpgrade={() => navigate('subscription')} onNavigate={navigate} />;
