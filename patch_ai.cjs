...
// 1. Replace the systemInstruction
...
const newInstruction = `If the user asks about buying tokens, NIN verification, or profile completion, guide them to go to their 'Profile' page via the bottom menu. 
...
// 2. Replace getLocalRouterReply entirely to handle the specific cases better.
...
4. Spent tokens let professionals instantly apply for high-budget marketplace jobs before others!\`;
    ...
    if (q.includes('verify') || q.includes('verification') || q.includes('nin') || q.includes('identity') || q.includes('badge') || q.includes('complete profile')) {
...
if (code.includes(oldInstruction) && oldRouter.includes('getLocalRouterReply')) {
    code = code.replace(oldInstruction, newInstruction);
    code = code.replace(oldRouter, newRouter);
    fs.writeFileSync('pages/Overview.tsx', code);
    console.log("Patched Overview.tsx AI assistant instructions.");
} else {
    console.log("Could not find targets to patch.");
    console.log("Found oldInstruction:", code.includes(oldInstruction));
    console.log("Found oldRouter length:", oldRouter.length);
}
