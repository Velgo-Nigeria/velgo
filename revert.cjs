...
// Revert individual buttons
code = code.replace(/px-3 md:px-4 py-1.5 md:py-2 bg-slate-900 hover:bg-slate-850 border border-slate-800 rounded-lg md:rounded-xl text-\[10px\] md:text-xs font-black uppercase text-white/g, 'px-4 py-2 bg-slate-900 hover:bg-slate-850 border border-slate-800 rounded-xl text-xs font-black uppercase text-white');

code = code.replace(/px-3 md:px-4 py-1.5 md:py-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-lg md:rounded-xl text-\[10px\] md:text-xs font-black uppercase text-slate-400 hover:text-white/g, 'px-4 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded-xl text-xs font-black uppercase text-slate-400 hover:text-white');

fs.writeFileSync('pages/AdminDashboard.tsx', code);
console.log("Reverted AdminDashboard mobile layout.");
