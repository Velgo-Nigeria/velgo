...
-- v3.0.0 - Production-Grade Triggers & Native Webhook Integration
-- =========================================================================
...
BEGIN
  ...
  BEGIN
    ...
    -- Asynchronously invoke Vercel serverless push API routing
    ...
    IF (TG_OP = 'UPDATE' AND OLD.status <> NEW.status AND (NEW.status = 'declined' OR NEW.status = 'cancelled')) THEN
        IF NEW.status = 'declined' THEN
            ...
            VALUES (
                ...
                worker_name || ' has declined your booking request.', 
                'alert'
            );
        ELSE
            INSERT INTO public.notifications (user_id, title, message, type)
